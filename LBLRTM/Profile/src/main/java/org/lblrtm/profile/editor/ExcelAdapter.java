...
public class ExcelAdapter implements ActionListener {
     ...
     */
    public void pasteValueAt(int row, int col, String value) {

        pasteValueAt(row, col, Double.parseDouble(value));
    }

    public void pasteValueAt(int row, int col, double val) {

        DJProfile p = ((DJProfile) ((ProfileTableModel) jTable1.getModel()).getProfile());
        p.setValueAt(row, col, val);
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getActionCommand().compareTo("Paste") == 0) {
            ...
            try {
                ...
                StringTokenizer st1 = new StringTokenizer(trstring, tS);
...
//                        
//                        ((DJProfile)((ProfileTableModel)jTable1.getModel()).getProfile()).createObject();
//                    }
...
//                            
//                            (((ValueUnitCell)jTable1.getValueAt(startRow+i, startCol+j))).setValue(Double.parseDouble(value));
//                        }
//                        //System.out.println("Putting " + value + "atrow=" + startRow + i + "column=" + startCol + j);

                        pasteValueAt(startRow + i, startCol + j, value);
                    }
                }
            } catch (Exception ex) {
                ex.printStackTrace();
